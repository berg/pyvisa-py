...
class FakeHiSLIPServer:
    ...
    def __init__(self, response=b"FAKE,INSTRUMENT\n", status_byte=0x00):
        ...
        #: When set to a control code, refuse traffic with an Error carrying it.
...
class TestTermChar:
    def test_termchar_splits_response(self, server):
        ...
        try:
            ...
            # It carries the END indicator, which VPP-4.3 RULE 6.1.1 ranks
            # above the termination character they arrived on together.
            data, status = sess.read(4096)
    ...
    def test_suppress_end_withholds_success(self, server):
        # VPP-4.3 RULE 6.1.4: VI_SUCCESS is unavailable while
        # VI_ATTR_SUPPRESS_END_EN is set. HiSLIP ends a response with a
        # DataEND message rather than an indicator riding on a byte, so there
        # is nothing further to read and only the status can honor the rule.
        sess = TCPIPInstrHiSLIP(
            0, f"TCPIP0::127.0.0.1::hislip0,{server.port}::INSTR", open_timeout=5000
        )
        try:
            sess.set_attribute(ResourceAttribute.suppress_end_enabled, True)
            sess.write(b"*IDN?\n")
    ...
    def test_suppress_end_leaves_termination_character_status(self, server):
        # RULE 6.1.2 is untouched by RULE 6.1.4: a read the termination
        # character stopped still reports VI_SUCCESS_TERM_CHAR.
        server.response = b"first\nsecond\n"
        sess = TCPIPInstrHiSLIP(
            0, f"TCPIP0::127.0.0.1::hislip0,{server.port}::INSTR", open_timeout=5000
        )
        try:
            ...
            # The tail carries the termination character and END together.
            # With the attribute clear RULE 6.1.1 promotes this to VI_SUCCESS;
            # RULE 6.1.4 withholds that, so the termination character answers.
            data, status = sess.read(4096)
            assert data == b"second\n"
            assert status == StatusCode.success_termination_character_read
        finally:
    ...
    def test_suppress_end_leaves_max_count_status(self, server):
        # RULE 6.1.3 is likewise untouched.
        sess = TCPIPInstrHiSLIP(
            0, f"TCPIP0::127.0.0.1::hislip0,{server.port}::INSTR", open_timeout=5000
        )
        try:
...
class TestRemoteLocal:
    ...
    def test_invalid_mode(self, session):
        assert session.gpib_control_ren(99) == StatusCode.error_nonsupported_operation
...
class TestServerRefusal:
    ...
    so dropping it leaves the caller waiting out its timeout with no
    diagnostic.
    """
...
class TestServiceRequests:
    ...
    def test_handler_may_read_stb(self, session, server):
        """The usual SRQ reaction — reading the status byte — must not hang.

        The handler runs on its own thread so it can talk to the instrument
        while the channel reader stays free to deliver the reply.
        """
